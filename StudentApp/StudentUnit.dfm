object StudentsForm: TStudentsForm
  ...
  object DataSetStudents: TADODataSet
    ...
    CursorType = ctStatic
    CommandText = 
      'SELECT s.PkStudent, s.FkPerson, s.FacultyNumber,'#13#10'(p.FirstName +' +
      ' '#39' '#39' + p.MiddleName + '#39' '#39' + p.LastName) AS PersonName,'#13#10'p.TypeId' +
      'Number, p.IDNumber, s.AcademicYearStart, s.AcademicYearEnd,'#13#10's.S' +
      'Status, s.FkProgram'#13#10'FROM Students '#13#10'AS s INNER JOIN Persons AS ' +
      'p ON s.FkPerson = p.PkPerson'#13#10'ORDER BY s.FacultyNumber ASC'
    DataSource = DataSourcePrograms
