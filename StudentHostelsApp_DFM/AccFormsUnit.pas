...
uses DataModuleUnit, SearchPersonUnit, PersonsUnit, HostelsRoomsInvertoryUnit;
...
begin
  ...
  begin
	  ...
	  //случай бутон ОК ще бъде забранен. Сега настройваме параметъра FkPerson
	  if (DataSetAccForms.Active) then DataSetAccForms.Close();
	  DataSetAccForms.Parameters.ParamByName('FkPerson').Value :=
...
begin
  var f: THostelsRoomsInventoryForm := THostelsRoomsInventoryForm.Create(Self);
  f.Width := 415;
  ...
  begin
    ...
    begin
	  ...
	  DataSetAccFormsFkRoom.AsInteger := f.DataSetRoomsPkRoom.AsInteger;
  end;
  f.Free();
end;
